...
from util import rotate_points, cal_pref_velocity, all_arrive_target_pos, cal_target_pos
...
class policyNet(nn.Module):
    ...
    def forward(self, x):
        ...
        mean_ = 50.0 * torch.tanh(self.fc_mu(x))
        # np.log(1 + np.exp(2))
...
class Config:
    ...
    minimal_size = 1024
    batch_size = 64
    save_path = os.getcwd() + r'/ppo_model.ckpt'
    # 回合停止控制
    max_episode_rewards = 0
    max_episode_steps = 400

    def __init__(self, env):
        self.state_dim = env.observation_dim + 2
        try:
...
def train_agent(env, cfg):
    ...
    for i in tq_bar:
        ...
        s = torch.cat((env.pxy, env.vxy, target_posi_tensor), dim=0)
        s = s.view(6, ).numpy()
        ...
        for i in range(cfg.max_episode_steps):
            ...
            # n_s: next state
            target_posi_tensor = torch.tensor(target_pos[env_i], dtype=torch.float).view(-1, 1)
            n_s = torch.cat((env.pxy, env.vxy, target_posi_tensor), dim=0)

            # r: reward
            r = cal_reward2(s, n_s)

            # buffer push
            n_s = n_s.view(6, ).numpy()
...
def cal_reward(state):
    ...
    #     distance = (state_2 - obs).T @ (state_2 - obs)
    #     if distance > 5:
    #         reward2 = 0
    #     else:
    #         reward2 = - (5 - distance) / (distance + 0.5)
    #
...
def cal_reward2(state, next_state):
    coef1, coef2 = 0.1, 0.5
    reward_sum = 0
...
if __name__ == '__main__':
    env_i = 0
    time_step = 1 / 10.
    env = USV((5.0, 5.0), time_interval=time_step)
