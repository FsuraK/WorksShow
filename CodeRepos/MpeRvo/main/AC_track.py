...
import torch
from env.UsvBaseEnv_numpy import USV
import numpy as np
...
class Tracking:
    def __init__(self, lr=0.01, model_path=None, input_dim=4, output_dim=2, device=torch.device('cpu')):
    ...
    def cal_tau(self, state):
        state = torch.tensor(state.T, dtype=torch.float32, requires_grad=True)
        tau = self.model(state)
        return tau

    def cal_u_2dim(self, state):
        state = torch.tensor(state.T, dtype=torch.float32, requires_grad=True)
        tau = self.model(state)
        zero = torch.tensor([0.0])
        tau_ = torch.cat((tau[:1], zero, tau[1:]))
...
if __name__ == "__main__":
    # init
    now_path = os.getcwd()
    save_path = now_path + '/save/AC_track_model0.pt'
