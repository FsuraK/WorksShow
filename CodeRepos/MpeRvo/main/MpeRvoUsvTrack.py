...
from plot.plot import plot_traj, plot_traj_static, plot_est_traj
...
def reset_sim_pos_speed():
    sim.setAgentPosition(usv0, (20, 20))
    sim.setAgentPosition(usv1, (40, 20))
    sim.setAgentPosition(usv2, (40, 40))
    sim.setAgentPosition(usv3, (20, 40))
...
# INIT USV
usv0 = sim.addAgent((20, 20))
usv1 = sim.addAgent((40, 20))
usv2 = sim.addAgent((40, 40))
usv3 = sim.addAgent((20, 40))
...
model_save_path = os.path.dirname(os.getcwd()) + "/save/TrackModel/track_NN_USV.pt"
model_base_path = os.path.dirname(os.getcwd()) + "/save/TrackModel/track_NN_USV_base.pt"
if_train = False
# if_train = True
...
env0 = USV((20.0, 20.0, 0.0), (0.0, 0.0, 0.0), time_interval=time_step)

"""INIT Speed_Estimation NN "first based on control u and now state=vel+pos for test"""
if_train_est = False
# if_train_est = True
est_model_save_path = os.path.dirname(os.getcwd()) + "/save/VelEstModel/Est_NN_linear_USV_random_u.pt"
est_model = None if if_train_est else est_model_save_path
est_NN = Tracking(lr=1e-5, input_dim=9, output_dim=3, model_path=est_model)
...
if if_train or if_train_est:
    for episode in tq_bar:
        ...
        for step in range(steps):
            ...
            tau[0] = random.randint(-50, 100)
            tau[2] = random.randint(-50, 100)
            """estimate set"""
            vxy_tmp = env0.v.detach().T[0]
            pxy_tmp = env0.eta.detach().T[0]

            env0.step(tau)

            true_speed_next, _ = env0.cal_vxy()
    ...
    for step in range(steps):
        ...
        vxy_env_lst.append(env0.v.detach().T[0].tolist())
    ...
    # plot_traj_static(x_lst, y_lst, obst_lst, num_agents=num_agents, circle_radius=e_radius, xy_lim=[10, 50])
    plot_est_traj(vxy_est_lst, vxy_env_lst, save_path=os.path.dirname(os.getcwd()) + "/save/ImgSave"
                                                                                     "/velocity_estimate_1.pdf")
