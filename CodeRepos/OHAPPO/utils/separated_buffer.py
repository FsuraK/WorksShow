...
class SeparatedReplayBuffer(object):
    """
    只传递进来列表,目的只是得到shape和判断离散还是连续
    也就是说这里传递进来的所有..._spcae,都不是实际的数据
    """
    def __init__(self, args, obs_space, share_obs_space, act_space):  # 只传递进来列表,目的只是得到shape
        ...
        # 此两条代码的结果--> obs_shape=obs_space， share_obs_shape=share_obs_space
        obs_shape = get_shape_from_obs_space(obs_space)
        share_obs_shape = get_shape_from_obs_space(share_obs_space)

        if type(obs_shape[-1]) == list:
            obs_shape = obs_shape[:1] # 切片，只取列表中第一个数值，这里返回252

        if type(share_obs_shape[-1]) == list:
            share_obs_shape = share_obs_shape[:1]  # 返回299， share_obs_shape中第一个数值是299

        # 401x8x299  401 = episode_length + 1 , 8 = n_rollout_threads并行数量， 299 = share_obs_shape纬数
        """为什么这里用401 = episode_length + 1呢，因为在smac_runner中，有一个step
            每一轮episode_length使用[step]只会取一个step，所以实际使用的是1x8x299"""
        ...
        # 401x8x1x64 （下2都是）  所使用神经网络的从input--->ouput使用的状态信息
    ...
    def compute_returns(self, next_value, value_normalizer=None):
        ...
        """
        if self._use_proper_time_limits:  # False
            if self._use_gae:
                ...
                for step in reversed(range(self.rewards.shape[0])):
                    if self._use_popart or self._use_valuenorm:
                        ...
                        gae = gae * self.bad_masks[step + 1]  # GAE计算完毕
            ...
            if self._use_gae:  # True
                self.value_preds[-1] = next_value  # 刚刚计算的最后一个step的next_value
                gae = 0
                for step in reversed(range(self.rewards.shape[0])):  # 399->0
                    """倒着step是因为gae计算的特性，第一个被计算的是gamma和lambda次方最多的，因此第一个必须是最后一个step"""
                    if self._use_popart or self._use_valuenorm:
    ...
    def feed_forward_generator(self, advantages, num_mini_batch=None, mini_batch_size=None):
        # 从reward中获取维度信息，就是最开始定义的信息 400 8
        episode_length, n_rollout_threads = self.rewards.shape[0:2]
        batch_size = n_rollout_threads * episode_length  # batch_size: 3200

        if mini_batch_size is None:  # go  mini_batch_size=None
            # 条件为真，跳过
            assert batch_size >= num_mini_batch, (
                ...
                "".format(n_rollout_threads, episode_length, n_rollout_threads * episode_length,
                          num_mini_batch))
            mini_batch_size = batch_size // num_mini_batch  # 3200

        rand = torch.randperm(batch_size).numpy()
        sampler = [rand[i*mini_batch_size:(i+1)*mini_batch_size] for i in range(num_mini_batch)]
        # sampler和rand中的数据一样，i从0开始，这一步是为了划分多个mini_batch，但我们这里num_mini_batch=1,sampler只有sampler[0]

        # 把并行环境这个维度灭掉，所有的并行环境归根就是为了训练，到此并行环境的使命就结束了
        # 400x8x299 ---> 3200x299, 400x8x252 ---> 3200x252, 400x8x1x64 ---> 3200x1x64, ..., 400x8x1 ---> 3200x1, ...
        # 再回顾一下[:-1] -> 比如obs，真正需要的是前400个obs，第401个数据是为了存储obs[400]执行action[400]的next_obs等...
        share_obs = self.share_obs[:-1].reshape(-1, *self.share_obs.shape[2:])
        obs = self.obs[:-1].reshape(-1, *self.obs.shape[2:])
        rnn_states = self.rnn_states[:-1].reshape(-1, *self.rnn_states.shape[2:])
        rnn_states_critic = self.rnn_states_critic[:-1].reshape(-1, *self.rnn_states_critic.shape[2:])
        actions = self.actions.reshape(-1, self.actions.shape[-1])
        if self.available_actions is not None:  # go
        ...
        # 注意sampler是个列表，sampler[0]包含3200个元素，这里for只会走一次，因为sampler只有一个元素sampler[0]
        # 因此，indices=array([..., ..., 3200个])
        for indices in sampler:
            # obs size [T+1 N Dim]-->[T N Dim]-->[T*N,Dim]-->[index,Dim]
            """这里是根据indices索引抽样，抽样出num_mini_batch个batch，但我们这里num_mini_batch=1，相当于全部抽取
            前面打乱sampler的原因，是为了多个batch的随机抽样"""
            ...
            # 这里迭代器的合理性：是因为在for循环内。在我们现在的工程里只循环一次，那这个yield其实就相当于return
            # 否则，可以直接进入循环，产生下一次的抽样数据，可以节省空间速度。
