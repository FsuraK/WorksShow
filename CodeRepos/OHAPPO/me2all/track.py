import os
from me2all.BaseAndUtils import net
import torch
from me2all.USVEnv import Single_USV_Unlin_Env
from me2all.EnvArgs import get_config
import numpy as np
import matplotlib.pyplot as plt
import time


def plot4(pxy_lst, exy_lst):
    # data
    px = [row[0] for row in pxy_lst]
    py = [row[1] for row in pxy_lst]
    ex = [row[0] for row in exy_lst]
    ey = [row[1] for row in exy_lst]

    # x-axis and plot init
    # fig, axs = plt.subplots(2, 2, figsize=(12, 8))
    fig, axs = plt.subplots()

    # plot 1
    axs.plot(px, py, label='pxy')
    axs.plot(ex, ey, label='exy')
    axs.set_title("--traj--")
    axs.legend()
    axs.set_xlim([-5, 70])
    axs.set_ylim([-5, 70])

    # plt.subplots_adjust(hspace=0.3, wspace=0.3)
    # fig.suptitle("Tracking effect", fontsize=20)
    plt.savefig(os.getcwd() + '/track_result1.pdf')
    plt.show()

class Tracking:
    def __init__(self, lr=0.01, model_path=None, input_dim=4, output_dim=2):
        self.model = net(input_dim=input_dim, output_dim=output_dim)
        self.model_path = model_path
        self.optimizer = torch.optim.Adam(self.model.parameters(), lr=lr)
        self.loss_fn = torch.nn.MSELoss()

        if self.model_path is not None:
            self.model.load_state_dict(torch.load(model_path))
            self.model.eval()

    def train(self, loss):
        self.optimizer.zero_grad()
        loss.backward()
        # print(loss.grad)
        self.optimizer.step()

    def save_model(self, model_path):
        torch.save(self.model.state_dict(), model_path)

    def cal_u(self, state):
        state = torch.tensor(state.T, dtype=torch.float32, requires_grad=True)
        u = self.model(state)
        return u


if __name__ == "__main__":
    # env args
    env_arg = ["--n_agents", '1', "--x_dim", '4', "--u_dim", '2', "--pxy_dim", '2', "--time_interval", '0.1']
    env_base_arg = get_config()
    env_arg = env_base_arg.parse_known_args(env_arg)[0]

    # init
    now_path = os.getcwd()
    save_path = now_path + '/save_path/USV_track_model2.pt'
    model_path = save_path
    # model_path = None
    env = Single_USV_Unlin_Env(env_arg)
    trackNN = Tracking(lr=5e-6, model_path=model_path, input_dim=6, output_dim=2)
    steps = 200
    episodes = 3000
    time_zero = time.time()
    time_end = time.time()
    device = torch.device("cuda:0") if torch.cuda.is_available() else torch.device("cpu")

    # episode start
    if model_path is None:
        for episode in range(episodes):
            x_now = env.reset()
            exy = torch.zeros((env_arg.pxy_dim, 1), dtype=torch.float32)
            exy[0] = 10
            exy[1] = 10
            episode_loss = 0

            # steps start
            for _ in range(steps):
                state = torch.cat([x_now, exy])
                u = trackNN.model(state.T) * 10
                x_next, done, infos = env.step(x_now, u)
                # loss cal  &  update
                loss = (x_next[:2] - exy).T @ (x_next[:2] - exy)
                trackNN.train(loss)

                # position update
                episode_loss = episode_loss + loss
                # exy[0] = exy[0] + 0.2
                # exy[1] = exy[1] + 0.2
                e_path_1 = 1
                if exy[1] <= 30 and (exy[0] <= 10):
                    e_path_1 = 0

                if e_path_1 == 1:
                    theta0 = np.arccos(exy[0] / 20 - 1.5)
                    theta = theta0 if exy[1] > 30 else -theta0
                    exy[0] = 30 + 20 * np.cos(theta - 0.6 / 20)
                    exy[1] = 30 + 20 * np.sin(theta - 0.6 / 20)
                else:
                    exy[1] = exy[1] + 0.6
                x_now = x_next.detach()
            if episode % 20 == 0:
                time_tmp = time_end
                time_end = time.time()
                print("episode =", episode+1, "  episode_loss =", episode_loss.tolist()[0][0],
                      "  time =", round(time_end-time_tmp, 2),
                      "  time_sum =", round(time_end-time_zero, 2))
            if (episode % 800 == 0) or (episode == episodes-1):
                trackNN.save_model(save_path)
    else:
        x_now = env.reset()
        exy = torch.ones((env_arg.pxy_dim, 1), dtype=torch.float32)
        exy[0] = 10
        exy[1] = 10
        pxy_lst = []
        exy_lst = []
        pxy_lst.append(x_now[:2].tolist())
        exy_lst.append(exy.tolist())
        for _ in range(steps):
            state = torch.cat([x_now, exy])
            u = trackNN.model(state.T) * 10
            x_next, done, infos = env.step(x_now, u)

            x_now = x_next
            # position update
            # exy[0] = exy[0] + 0.1
            # exy[1] = exy[1] + 0.1
            """evader 圆形轨迹"""
            e_path_1 = 1
            if exy[1] <= 30 and (exy[0] <= 10):
                e_path_1 = 0

            if e_path_1 == 1:
                theta0 = np.arccos(exy[0] / 20 - 1.5)
                theta = theta0 if exy[1] > 30 else -theta0
                exy[0] = 30 + 20 * np.cos(theta - 0.6 / 20)
                exy[1] = 30 + 20 * np.sin(theta - 0.6 / 20)
            else:
                exy[1] = exy[1] + 0.6

            # lst store for plt
            pxy_lst.append(x_now[:2].tolist())
            exy_lst.append(exy.tolist())
        plot4(pxy_lst, exy_lst)
